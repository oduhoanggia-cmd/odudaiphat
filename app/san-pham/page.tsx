...
import { categories, products, getCategory, productsByCategory } from '@/lib/products';
...
export default function ProductsPage({ searchParams }: { searchParams?: { 'danh-muc'?: string } }) {
  const category = searchParams?.['danh-muc'] ? getCategory(searchParams['danh-muc']) : undefined;
  const displayProducts = category ? productsByCategory(category.slug) : products;
    ...
    <main>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-8 flex gap-3 overflow-x-auto pb-2">
          ...
          {categories.map((cat) => (
            <Link key={cat.slug} href={`/san-pham?danh-muc=${cat.slug}`} className={`whitespace-nowrap rounded-full px-5 py-3 text-sm font-bold ${category?.slug === cat.slug ? 'bg-blue-700 text-white' : 'bg-white text-slate-700 ring-1 ring-slate-200'}`}>{cat.name}</Link>
          ))}
        </div>
